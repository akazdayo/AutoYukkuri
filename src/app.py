...
class App:
    ...
    def run(self, model: str, input_path: str, output_path: str, speaker_switch, page: ft.Page) -> None:
        ...
        if speaker_switch:
            ...
            print("出力中")
            self.status_checker.checker("出力中")
            proj = self.process.write(result, [0 for i in range(
                len(result["segments"]))], self.characters)
